...
int execute4(char* argument[], int max, int ignore[], char* redirect[])
/*
	...
	for(i; i < max; i++) {
		...
		if(child == 0) {// Child
			...
			if(redirect[0] != NULL && i == 0) {
				...
				dup2 (filefd_in, 0);        // read for stdin
			}
			if(redirect[1] != NULL && i == max - 1) {
				int filefd_out = open(redirect[1], O_WRONLY | O_CREAT, 0644);
				dup2 (filefd_out, 1);       // read from stdout
			}
