...
void * writer(void * in_ptr) {
	...
	typedef char* (c)[100];			// name as 'c' change in_ptr as char rather than char* why????
	int write = 0;					// how many commend are being written

	// loop untile the write equal the number of element in buffer
	while (1) {
		if (get_written() == 0){
			// get_written = 0 when second thread finishe execute bed_read (written == 0)
			// unlock
			bad_write((*((c*)in_ptr))[write]);		// brainfked
			write++;
		}
		if (write == total_commands){
			break;
		}
	}
	return NULL;
...
void * reader(void * empty) {
	...
	int read = 0;					// define a check num break untile read = total_cmds

	// loop nutil read all
	while (1) {
		if (get_written() == 1){
			// written = 1 when finish execute writer
			// unlock
			bad_read(empty);
			read++;
		}
		if (read == total_commands){
			// read all!
			break;
		}
	}
